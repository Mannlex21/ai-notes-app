...
const handleRegister = async () => {
	...
	try {
		...
		errorMsg.value = err.message || "Ocurrió un error al crear la cuenta.";
	} finally {
		loading.value = false;
	}
};
...
<template>
	<div class="min-h-screen bg-[#f7f4ea] flex items-center justify-center p-4">
		<div
			class="max-w-md w-full bg-[#f2eee3] border border-[#d8d3c5] rounded-2xl p-8 shadow-sm space-y-6"
		>
			<div class="text-center space-y-2">
				<div
					class="w-10 h-10 rounded-xl bg-[#3d3b37] text-[#f7f4ea] font-serif font-bold text-xl flex items-center justify-center mx-auto"
				>
					N
				</div>
				<h2 class="text-2xl font-serif font-bold text-[#3d3b37]">
					Crear una cuenta
				</h2>
				<p class="text-xs text-[#8c867a]">
					Empieza a organizar tus notas hoy mismo
				</p>
			</div>
			...
			<form @submit.prevent="handleRegister" class="space-y-4">
				<div
					v-if="errorMsg"
					class="p-3 bg-red-100 border border-red-300 text-red-700 text-xs rounded-lg"
				>
					{{ errorMsg }}
				</div>
			...
			<div class="text-center text-xs text-[#8c867a]">
				...
				<RouterLink
					to="/login"
					class="text-[#3d3b37] font-semibold underline"
