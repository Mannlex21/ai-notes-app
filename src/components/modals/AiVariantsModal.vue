...
<template>
	...
	>
		<div
			class="bg-[#f7f4ea] border border-[#d8d3c5] rounded-xl p-5 shadow-2xl max-w-lg w-full space-y-4"
		>
			<div
			...
			>
				...
				>
					<Sparkles class="w-4 h-4 text-[#e06c53]" /> Selecciona la
			...
			<div class="space-y-2.5 max-h-[60vh] overflow-y-auto pr-1">
				...
				>
					<!-- Renderizado de HTML con v-html -->
					<div
						class="variant-preview text-xs text-[#3d3b37] leading-relaxed flex-1"
						v-html="variant"
					></div>
					<span
						class="opacity-0 group-hover:opacity-100 text-[#e06c53] transition-opacity shrink-0 pt-0.5"
					>
						<Check class="w-4 h-4" />
					</span>
				</div>
...
<style scoped>
/* Estilos para renderizar listas y negritas correctamente en la vista previa */
:deep(.variant-preview ul) {
...
:deep(.variant-preview li) {
	margin-bottom: 0.2rem;
}

:deep(.variant-preview strong) {
	font-weight: 600;
	color: #2a2926;
}
</style>
