...
<template>
	<div ref="menuRef" class="relative">
		...
		>
			<!-- Nueva Acción: Extraer Tareas -->
			<button
				@click.stop="handleAction(() => emit('extract-tasks'))"
				class="w-full text-left px-3 py-2 hover:bg-[#e8e3d5] flex items-center gap-2 font-medium text-[#e06c53]"
			>
				<ListCheck class="w-3.5 h-3.5 text-[#e06c53]" /> Extraer tareas
				(Checklist)
			</button>
