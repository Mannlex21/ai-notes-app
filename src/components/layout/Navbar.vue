...
<template>
  <header class="h-14 border-b border-[#d8d3c5] bg-[#f2eee3] px-4 flex items-center justify-between sticky top-0 z-30">
    <!-- Logotipo / Branding -->
    <div class="flex items-center gap-2">
      <div class="w-8 h-8 rounded-lg bg-[#3d3b37] text-[#f7f4ea] flex items-center justify-center font-serif font-bold text-base shadow-sm">
        N
      </div>
      <span class="font-serif font-semibold text-base text-[#3d3b37] hidden sm:inline">PaperNotes</span>
    </div>
    ...
    <div class="flex-1 max-w-md mx-4">
      <div class="relative">
        <input 
          type="text" 
          placeholder="Buscar notas o pedir a IA..." 
          class="w-full bg-[#f7f4ea] border border-[#d8d3c5] rounded-lg pl-9 pr-3 py-1.5 text-xs text-[#3d3b37] placeholder-[#8c867a] focus:outline-none focus:border-[#3d3b37] transition-all"
        />
        <Sparkles class="w-4 h-4 text-[#8c867a] absolute left-2.5 top-1/2 -translate-y-1/2" />
      </div>
    </div>
      ...
      >
        ...
        >
          ...
          <div class="px-3.5 py-2 space-y-0.5">
            <p class="font-semibold truncate text-[#3d3b37]">{{ authStore.user?.fullName }}</p>
            <p class="text-[11px] text-[#8c867a] truncate">{{ authStore.user?.email }}</p>
          </div>
          ...
          <div class="py-1">
            ...
            >
              <Settings class="w-4 h-4 text-[#8c867a]" /> Configuración
            </button>
          </div>
          ...
          <div class="py-1">
            ...
            >
              <LogOut class="w-4 h-4 text-red-600" /> Cerrar sesión
            </button>
          </div>
        </div>
      </Transition>
